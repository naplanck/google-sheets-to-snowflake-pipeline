    ...
    for tbl, table_config in transform_rules.items():
        ...
        for sheet in table_config["SHEETS"]:
            for source_col, col_config in sheet["COLS"].items():
                validate_sql_identifier(col_config["NAME"], f"{tbl}.{source_col}.NAME")
        ...
        for sheet in table_config["SHEETS"]:
            mapped_cols = {col_config["NAME"] for col_config in sheet["COLS"].values()}

            missing_from_t_cols = mapped_cols - target_cols

            if missing_from_t_cols:
                raise ValueError(
                    f"Mapped columns missing from {tbl}.T_COLS: {sorted(missing_from_t_cols)}"
                )
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if status_code not in TRANSIENT_STATUS_CODES:
                logger.exception(
                    f"ERROR - Non-retryable Google Sheets API error. status_code={status_code}"
                )
                raise
...
def sheet_values_to_dataframe(header, column, values):
    ...
    sliced_rows = [row[column:] if len(row) > column else [] for row in values[header:]]
    ...
    data_rows = [row + [""] * (max_width - len(row)) for row in sliced_rows[1:]]
...
def align_to_target_schema(df, rules):
    ...
    required_source_cols = [
        source_col for source_col, rule in rules.items() if rule.get("REQUIRED", True)
    ]
    ...
    ignored_cols = [col for col in df.columns if col not in source_cols]
    ...
    rename_map = {old_col: name["NAME"] for old_col, name in rules.items()}
...
def build_table_dataframe(client, table_config):
    ...
    aligned_dfs = [df.reindex(columns=all_columns, fill_value=pd.NA) for df in dfs]
